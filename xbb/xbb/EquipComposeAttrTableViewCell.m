...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (0 == indexPath.row) {
        ...
        NSString *attrSepStr=@"+";
        if (attrDoubleValue < 0) {
            attrSepStr=@"-";
            
            attrIntValue=abs(attrIntValue);
            attrDoubleValue=fabs(attrDoubleValue);
        }
        if (attrIntValue == attrDoubleValue) {
            attrItemCell.lblAttrDesc.text=[NSString stringWithFormat:@"%@ %@ %d%@",attrDesc.attrDesc,attrSepStr,attrIntValue,withPercentSufix?@"\%":@""];
        } else {
            attrItemCell.lblAttrDesc.text=[NSString stringWithFormat:@"%@ %@ %.1f%@",attrDesc.attrDesc,attrSepStr,attrDoubleValue,withPercentSufix?@"\%":@""];
        }
    }
